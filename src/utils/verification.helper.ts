...
export const validatePhone = (value: string): boolean => {
    const phoneValidation = Joi.string().regex(/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/im).validate(value);
...
export const validatePassword = (value: string): boolean => {
    const passwordValidation = Joi.string().regex(/^(?=.[A-Za-z])(?=.\d)(?=.[@$!%#?&])[A-Za-z\d@$!%*#?&]{8,}$/).validate(value);
...
export const validateEmail = (value: string): boolean => {
    const emailValidation = Joi.string().regex(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/).validate(value);
...
export const validateUsername = (value: string): boolean => {
    const userNameValidation = Joi.string().min(3).max(20).regex(/^[a-zA-Z0-9]+$/).validate(value);
...
export const validateName = (value: string): boolean => {
    const nameValidation = Joi.string().min(2).max(30).regex(/^[a-zA-Z]+$/).validate(value);
